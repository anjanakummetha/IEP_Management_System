  ...
  async function handleRecalculate() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCurrentReasons(json.reasons);
        router.refresh();
      }
    } finally {
  ...
  return (
    <Card>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm mb-4">
        ...
        <div>
          <p className="text-slate-500 font-medium">Compliance Risk</p>
          <div className="mt-1 flex items-center gap-2">
            <RiskBadge level={level} large />
            <span className="text-xs text-slate-400">{currentScore}/100</span>
          </div>
        </div>
      </div>
        ...
        <div className="border-t border-slate-100 pt-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm font-medium text-slate-700">Compliance Flags</p>
            </div>
            {studentId && (
              <button
                type="button"
